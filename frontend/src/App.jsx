      ...
      <aside className={`sidebar ${mobileOpen ? "open" : ""}`}>
        ...
        <div className="sidebar-bottom">
          <div className="engine-row">
            <span className="dot online" /> API ENGINE <b>ONLINE</b>
          </div>
          <div className="engine-row">
            <span className="dot online" /> ML ENGINE <b>READY</b>
          </div>
        ...
        <header className="topbar">
          <div>
            <div className="eyebrow">
              FORENSIC OPERATIONS / {page.toUpperCase()}
            </div>
    ...
    <section className="page">
      <div className="hero">
        <div>
          ...
          <p>
            Centralized forensic evidence handling, timeline reconstruction
            and behavioral anomaly analysis.
          </p>
        </div>

        <div className="hero-badge">
          <ShieldCheck size={18} /> FORENSIC CORE ACTIVE
        </div>
      </div>
      ...
      <div className="stats-grid">
        ...
        <Stat
          icon={AlertTriangle}
          label="Anomalies"
          value={anomalous.length}
          danger={anomalous.length > 0}
        />
      ...
      <div className="content-grid">
        <Panel
          ...
          subtitle="Unified forensic event stream"
        ...
        >
          <div className="event-list">
            {recent.length
              ...
              : <Empty text="No timeline events available." />}
          </div>
        </Panel>

        <Panel
          title="Behavioral Signal"
          subtitle="Isolation Forest analysis"
          action="OPEN ANALYSIS"
          onAction={() => onNavigate("behavior")}
        >
          {anomalous.length ? (
            <div className="alert-card">
              ...
              <div>
                <strong>
                  {anomalous.length} anomalous pattern
                  {anomalous.length > 1 ? "s" : ""} detected
                </strong>
            ...
            <div className="success-card">
              ...
              <span>No anomalous patterns detected.</span>
            </div>
          )}
          ...
          <div className="signal-bars">
            <SignalBar
              ...
              value={normal.length}
              max={Math.max(anomalous.length + normal.length, 1)}
            />
...
function EvidencePage() {
  ...
  const handleUpload = async (file) => {
    ...
    try {
      setResult(await uploadEvidence(file));
    } catch (e) {
      setError(e.message);
    } finally {
      setBusy(false);
    ...
    <section className="page">
      <div className="section-intro">
        <div>
          ...
          <p>
            Securely submit forensic artifacts and capture their cryptographic
            fingerprint.
          </p>
        </div>
      </div>
      ...
      <div className="upload-card">
        ...
        <p>
          Any file supported by the backend can be submitted for SHA-256
          hashing and storage.
        </p>
      ...
      {result && (
        <div className="result-card">
          <div className="result-head">
            <CheckCircle2 />
            <span>Evidence stored successfully</span>
          </div>
          ...
          <div className="hash-row">
            <span>STATUS</span>
            <strong className="verified">VERIFIED FOR STORAGE</strong>
          </div>
        </div>
      )}
    </section>
    ...
    <section className="page">
      <div className="section-intro">
        <div>
          ...
          <p>
            Chronological reconstruction of authentication, file access and
            network activity.
          </p>
        </div>
    ...
    <section className="page">
      <div className="section-intro">
        <div>
          ...
          <p>
            Isolation Forest classification with human-readable detection
            factors.
          </p>
        </div>
      </div>
      ...
      <div className="analysis-grid">
        {anomalies.map((a, i) => (
          <div
            className={`analysis-card ${a.status === "ANOMALOUS" ? "threat" : ""}`}
            key={i}
          >
            ...
            <div className="feature-grid">
              ...
              <Feature
                ...
                value={Number(a.bytes_transferred).toLocaleString()}
              />
              <Feature
                label="NEW IP"
                value={a.new_ip ? "DETECTED" : "NO"}
              />
            </div>
...
function IntegrityPage() {
  ...
  const verify = async (e) => {
    ...
    try {
      setResult(await verifyEvidence(filename, hash));
    } catch (err) {
      setResult({
        status: "ERROR",
        message: err.message
    ...
    <section className="page">
      <div className="section-intro">
        <div>
          ...
          <p>
            Compare an evidence artifact against its recorded SHA-256 hash.
          </p>
        </div>
      </div>
...
function EventRow({ event, detailed }) {
  ...
  return (
    <div className={`event-row ${detailed ? "detailed" : ""}`}>
      <div className={`event-icon ${type.toLowerCase()}`}>
        <Network size={16} />
      </div>

      <div className="event-main">
        <strong>{event.action || type}</strong>
        <span>{event.details || event.user || "Forensic event"}</span>
      </div>
